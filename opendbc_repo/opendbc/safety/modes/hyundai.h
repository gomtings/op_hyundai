...
#define HYUNDAI_COMMON_RX_CHECKS(legacy)                                                                                                                  \
  {.msg = {{0x260, 0, 8, 100U, .max_counter = 3U, .ignore_quality_flag = true},                                                                                           \
           {0x371, 0, 8, 100U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true},                                                             \
           {0x91,  0, 8, 100U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}}},                                                           \
...
static void hyundai_rx_hook(const CANPacket_t *msg) {
  ...
  if (bus == 0) {
    if (addr == 0x251) {
      int torque_driver_new = ((int)(GET_BYTES(msg, 0, 4) & 0x7ffU) - 982) * 0.4;
...
static safety_config hyundai_init(uint16_t param) {
  ...
  if (hyundai_longitudinal) {
    static RxCheck hyundai_long_rx_checks[] = {
      ...
      {.msg = {{0x4F1, 0, 4, 50U, .ignore_checksum = true, .max_counter = 15U}, { 0 }, { 0 }}},
    };
